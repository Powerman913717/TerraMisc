	...
	public void drawBlockHighlightEvent(DrawBlockHighlightEvent evt)
	{
		...
		else if (evt.currentItem != null && evt.currentItem.getItem() instanceof ItemCustomHoe && hoeMode == 3)
		{
			...
			if(b == TFCMBlocks.blockCrops && (
					...
					world.getBlock(evt.target.blockX,evt.target.blockY-1,evt.target.blockZ) == TFCBlocks.tilledSoil2))
			{
				TECropTFCM te = (TECropTFCM) world.getTileEntity(evt.target.blockX, evt.target.blockY, evt.target.blockZ);
				CropIndexTFCM index = CropManagerTFCM.getInstance().getCropFromName(te.cropId);
				boolean fullyGrown = index instanceof CropIndexRipenTFCM ? te.growth >= index.numGrowthStages - 1 : te.growth >= index.numGrowthStages;
